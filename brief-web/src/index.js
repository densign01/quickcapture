...
export default {
	async fetch(request, env, ctx) {
		...
		try {
            ...
            if (aiSummary) {
                const summaryText = await generateSummary(env, url, title, summaryLength);
...
async function generateSummary(env, url, title, summaryLength) {
    try {
        ...
        if (!articleResponse.ok) {
            ...
            return generateTitleBasedSummary(env, title, url, summaryLength);
        }
		...
		const hasPaywallIndicators = paywallIndicators.some((indicator) => lowercaseHtml.includes(indicator));
