...
# Universal build script for Brief app (supports both Apple Silicon and Intel)
set -e
...
echo "🏗️  Building Brief macOS App (Universal Binary)..."
...
echo "🔨 Compiling Swift files for multiple architectures..."
...
# Copy app icon and create proper icon bundle
...
hdiutil create -volname "Brief Installer" \
    -srcfolder "$DMG_DIR" \
    -ov -format UDZO \
    "${BUILD_DIR}/Brief-Universal-Installer.dmg"

echo "✅ Universal build complete!"
echo "📁 App: $APP_BUNDLE"
echo "💿 DMG: ${BUILD_DIR}/Brief-Universal-Installer.dmg"
echo "🔍 Architectures: $(lipo -info "${MACOS_DIR}/${APP_NAME}" | cut -d: -f2)"
echo ""
echo "This app will run natively on both Apple Silicon and Intel Macs!"
echo ""
echo "To install:"
echo "1. Open the DMG file"
echo "2. Drag Brief.app to your Applications folder"
echo "3. Right-click Brief.app → Open (first time only)"
