// Service worker for QuickCapture Safari extension (Manifest V3)
chrome.runtime.onInstalled.addListener(() => {
  console.log('QuickCapture extension installed');
  
  // Create context menu item (with error handling)
  try {
    if (chrome.contextMenus) {
      chrome.contextMenus.create({
        id: 'quickcapture-page',
        title: 'Send page to QuickCapture',
        contexts: ['page']
      });
    }
  } catch (error) {
...
chrome.action.onClicked.addListener((tab) => {
  ...
  console.log('QuickCapture clicked for tab:', tab.url);
});
...
if (chrome.contextMenus) {
  chrome.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === 'quickcapture-page') {
